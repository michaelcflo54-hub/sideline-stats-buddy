-- Add team_code field to teams table for joining functionality (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'teams' AND column_name = 'team_code') THEN
        ALTER TABLE public.teams ADD COLUMN team_code TEXT UNIQUE;
    END IF;
END $$;

-- Generate unique 6-character team codes for existing teams that don't have one
UPDATE public.teams 
...
-- Create policy to allow users to find teams by code (but only return minimal info)
